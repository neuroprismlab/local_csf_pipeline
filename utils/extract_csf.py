def extract_local_csf_mask( 
    ...
    binary_roi_nii = nib.load(binary_roi_path) # Original binary ROI mask
...
def extract_local_csf_time_series(
    ...
    local_csf_path : str
        ...
        (e.g., 'sub-011_csf_0.6_in_dilated_R_amygdala_TS_run-01.csv')
    ...
    local_csf_nii = nib.load(local_csf_path)         # Local CSF mask (binary NIfTI image)          
    ...
    column_label = f"{roi_name}_local_csf"
...
def add_local_csf_time_series_to_confound_file(
    ...
    local_csf_timeseries_df = pd.read_csv(local_csf_timeseries_path) # Load the CSF time series DataFrame

    csf_column_name = f"{roi_name}_local_csf"
